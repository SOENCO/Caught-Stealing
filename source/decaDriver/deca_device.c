...
int dwt_initialise(int config)
{
    ...
    temp = dwt_readdevid();
    if (DWT_DEVICE_ID != temp) // MP IC ONLY (i.e. DW1000) FOR THIS CODE
    {
        return DWT_ERROR ;
    }
...
void _dwt_loaducodefromrom(void)
{
    ...
    nrf_delay_ms(1); // Allow time for code to upload (should take up to 120 us)
...
void dwt_isr(void)
{
    ...
    if(status & SYS_STATUS_TXFRS)
    {
        ...
        {
			NRF_LOG_DEBUG("Debug: wait4resp force RX off");
    ...
    if(status & SYS_STATUS_ALL_RX_TO)
    {
		...
		NRF_LOG_DEBUG("Debug: SYS_STATUS_ALL_RX_TO force RX off");
        dwt_forcetrxoff();
        dwt_rxreset();
    ...
    if(status & SYS_STATUS_ALL_RX_ERR)
    {
        ...
        // the next good frame's timestamp is computed correctly.
        // See section "RX Message timestamp" in DW1000 User Manual.
		//NRF_LOG_DEBUG("Debug: SYS_STATUS_ALL_RX_ERR force RX off");
        dwt_forcetrxoff();
        dwt_rxreset();
		...
		//NRF_LOG_INFO("RX ERROR: %u", status);
    }
}
...
int dwt_starttx(uint8 mode)
{
    ...
    if (mode & DWT_START_TX_DELAYED)
    {
        ...
        else
        {
			...
			if (checkTxOK && (SYS_STATUS_HPDWARN >> 3)) {
            	retval = DWT_ERROR;	// HPDWARN
			}
			if (checkTxOK && (SYS_STATUS_TXPUTE >> 3)) {
            	retval = DWT_ERROR;	// TXPUTE
			}
